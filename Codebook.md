## Codebook for Samsung Activity Data
## Four columns below are
- Variable Number
- Original variable name from "features.txt"
- Valid variable name using make.names(featureNames, unique=TRUE, allow_=TRUE)
- Cleaned up variable name using gsub() to substitute "mean" for mean.." and "std" for "std.."

## Finally, at the end of the variable list, we have the cleaned up variables names and activity names. Relevant code is also given.
...
## Create data frame actMeanStd by deleting unnecessary variables using the dplyr library and the select function

## Now we just need to include activity names 
``````
...
``````
## Code above changes activity names as follows
"x"
"1" "WALKING WALKING"
...
## This code adds activity names to data frame actMeanStd
